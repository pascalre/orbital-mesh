...
// Hilfsfunktion außerhalb der Komponente, um Neuerstellungen zu vermeiden
function getPoints({ numStars = 500 } = {}) {
  ...
  const updateFn = (t) => {
    ...
    for (let i = 0; i < numStars; i += 1) {
      ...
      // Direkter Zugriff auf das Array ist schneller als setAttribute
...
// 1. memo verhindert Rerenders durch App-State-Änderungen (Hover)
const Starfield = memo(function Starfield() {
  const ref = useRef<THREE.Points>(null);

  // 2. useMemo stellt sicher, dass die 500 Sterne nur EINMAL gewürfelt werden
  const { points, updateFn } = useMemo(() => {
    console.log("✨ Starfield generiert");
    return getPoints({ numStars: 500 });
  }, []);

  // 3. useFrame läuft unabhängig vom React-Rendering
  useFrame((state) => {
    if (updateFn) {
